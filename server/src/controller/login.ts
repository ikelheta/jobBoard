...
export class LoginController {
  public static employeeLogin(body): Observable<any> {
    ...
    return of(true).pipe(
      ...
      mergeMap((m) => m ? of({ token: createToken({ ...user, type: "employee", }), id: user._id, type: "employee" }) : throwError(() => 401))
    )
  }
  //-----------------------------------------------------------------------------------------------------------------------------------------------------
  public static employeerLogin(body): Observable<any> {
    ...
    return of(true).pipe(
      ...
      tap((t) => user = t),
      mergeMap((m: any) => from(bcrypt.compare(password, m.password))),
      mergeMap((m) => m ? of({ token: createToken({ ...user, type: "employeer" }), id: user._id, type: "employeer" }) : throwError(() => 401))
    )
  }
}