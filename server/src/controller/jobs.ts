...
export class JobsController {
  ...
  public static getAllJobForEmployeer(pn: number, id: string): Observable<any> {
    return of(true).pipe(
      mergeMap(() => from(JobsSchema.find({ createdBy: id }).skip((pn - 1) * 10).limit(10).populate({ path: "applications rejected", select: ["-password"] }))),
      mergeMap((m) => {
        return forkJoin([
          of(m),
  ...
  public static getAllJobs(pn: number): Observable<any> {
    return of(true).pipe(
      mergeMap(() => from(JobsSchema.find({ status: "open" }).skip((pn - 1) * 10).limit(10).populate({ path: "createdBy", select: ["-password"] }).sort({ _id: -1 }))),
      mergeMap((m) => {
        return forkJoin([
          of(m),
